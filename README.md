...
- The builder combines raw inputs with fetched Yahoo Finance data cached in `data/2_fetched_data/`, writes processed artefacts to `data/3_processed_data/`, and saves the finished PPT in `output/`.
...
9. **Outputs** - The finished deck and supporting artefacts (classification review JSON, commentary history, chart images, news cache) land in `output/` and `data/3_processed_data/`.
...
commentary:
  ...
  rss_feeds:                      # Google News RSS feeds for market context
    ...
    # Add more Google News feeds as needed
  exclude_keywords:
...
2. **Market News** - Fetches general market news from Google News RSS, then AI filters to select top 10 most relevant articles for portfolio context
...
Set `enabled: false` to craft commentary manually, or adjust `max_macro_headlines` and `news_per_ticker` to control news volume.
